...
func TestMakeMove_WithUndo(t *testing.T) {
	...
	for _, tt := range games {
		t.Run(tt.link, func(t *testing.T) {
			...
			require.Equal(t, tt.fen, fen.Encode(board).String())
...
func TestMakeMove_UCIWithUndo(t *testing.T) {
	initFEN := fen.Encode(standardchess.NewBoard())
	...
	for _, tt := range games {
		t.Run(tt.link, func(t *testing.T) {
			...
			uciMoves := standardtest.UCIMovesFromSAN(pgn.Moves())
			for i, move := range uciMoves {
				result, err := board.MakeMove(move)
				require.NoErrorf(t, err, "move %s (%s) no %d", move, pgn.Moves()[i], i+1)
				require.NotNilf(t, result, "move %s (%s) no %d", move, pgn.Moves()[i], i+1)
			}
